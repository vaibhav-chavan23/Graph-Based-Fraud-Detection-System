    ...
    return (
        <div className="bg-white border border-sky-200 rounded-lg flex flex-col transition-all duration-300">
            <div className="bg-sky-50 p-4 border-b border-sky-200 flex justify-between items-center">
                <h2 className="font-bold text-slate-800">Cycle Detector</h2>
                <span className="text-xs font-mono bg-sky-100 text-sky-700 px-2 py-1 rounded">DFS</span>
            </div>
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center bg-slate-50 border border-slate-200 rounded p-3">
                            <div className="flex flex-col">
                                <span className="text-[10px] text-slate-500 font-semibold uppercase leading-tight">Total Cycles</span>
                                <span className="font-mono text-slate-800 text-sm">{summary.totalCyclesFound}</span>
                            </div>
                            <div className="flex flex-col border-x border-slate-200">
                                <span className="text-[10px] text-slate-500 font-semibold uppercase leading-tight">Longest</span>
                                <span className="font-mono text-slate-800 text-sm">{summary.longestCycleLength}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[10px] text-slate-500 font-semibold uppercase leading-tight">Average</span>
                                <span className="font-mono text-slate-800 text-sm">{summary.averageCycleLength?.toFixed(1)}</span>
                            </div>
                        </div>
                            ...
                            <div className="flex flex-col gap-2">
                                {topFindings.map((finding, idx) => (
                                    <div key={idx} className="text-sm flex gap-2">
                                        ...
                                        <span className="font-mono text-slate-700 flex-1 break-all">{finding.description}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                                        ...
                                        <tbody>
                                            {allFindings.map((finding, idx) => (
                                                <tr key={idx} className="border-b border-slate-100 last:border-0">
                                                    ...
                                                    <td className="py-2 pr-2 font-mono text-slate-700 break-all leading-tight">{finding.description}</td>
                                                    <td className="py-2 font-mono text-amber-600 text-right">{finding.riskContribution}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
