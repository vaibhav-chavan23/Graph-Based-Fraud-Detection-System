    ...
    return (
        <div className="bg-white border border-sky-200 rounded-lg flex flex-col transition-all duration-300">
            <div className="bg-sky-50 p-4 border-b border-sky-200 flex justify-between items-center">
                <h2 className="font-bold text-slate-800">Propagation Detector</h2>
                <span className="text-xs font-mono bg-sky-100 text-sky-700 px-2 py-1 rounded">BFS</span>
            </div>
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center bg-slate-50 border border-slate-200 rounded p-3">
                            <div className="flex flex-col">
                                <span className="text-[10px] text-slate-500 font-semibold uppercase leading-tight">Chains</span>
                                <span className="font-mono text-slate-800 text-sm">{summary.chainsFound}</span>
                            </div>
                            <div className="flex flex-col border-x border-slate-200">
                                <span className="text-[10px] text-slate-500 font-semibold uppercase leading-tight">Max Depth</span>
                                <span className="font-mono text-slate-800 text-sm">{summary.maxBfsDepth}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[10px] text-slate-500 font-semibold uppercase leading-tight">Avg Depth</span>
                                <span className="font-mono text-slate-800 text-sm">{summary.averageDepth?.toFixed(1)}</span>
                            </div>
                        </div>
