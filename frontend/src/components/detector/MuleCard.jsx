    ...
    return (
        <div className="bg-white border border-sky-200 rounded-lg flex flex-col transition-all duration-300">
            <div className="bg-sky-50 p-4 border-b border-sky-200 flex justify-between items-center">
                <h2 className="font-bold text-slate-800">Mule Detector</h2>
                <span className="text-xs font-mono bg-sky-100 text-sky-700 px-2 py-1 rounded">Degree Analysis</span>
            </div>
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-3 gap-2 text-center bg-slate-50 border border-slate-200 rounded p-3">
                            <div className="flex flex-col">
                                ...
                                <span className="font-mono text-slate-800 text-sm">{summary.muleCandidatesFound}</span>
                            </div>
                            <div className="flex flex-col border-x border-slate-200">
                                <span className="text-[10px] text-slate-500 font-semibold uppercase leading-tight">Max In-Deg</span>
                                <span className="font-mono text-slate-800 text-sm">{summary.highestInDegree}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[10px] text-slate-500 font-semibold uppercase leading-tight">Avg In-Deg</span>
                                <span className="font-mono text-slate-800 text-sm">{summary.averageInDegree?.toFixed(1)}</span>
                            </div>
                        </div>
                                        ...
                                        <tbody>
                                            {allFindings.map((finding, idx) => (
                                                <tr key={idx} className="border-b border-slate-100 last:border-0">
                                                    ...
                                                    <td className="py-2 font-mono text-amber-600 text-right">{finding.muleScore}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
